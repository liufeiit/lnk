 ...
 * 从而完成servivce的心跳。 开关开启后会进行心跳，开关关闭则停止心跳。
 * 
 * @author zhanglei
 *
 */
public class ConsulHeartbeatManager {
	protected static final Logger log = LoggerFactory.getLogger(ConsulHeartbeatManager.class);
	...
	private volatile boolean currentHeartBeatSwitcherStatus = true;
	// 开关检查次数。
	private int switcherCheckTimes = 0;

	public ConsulHeartbeatManager(LnkConsulClient client) {
		this.client = client;
	...
	public void setClient(LnkConsulClient client) {
		this.client = client;
	}
